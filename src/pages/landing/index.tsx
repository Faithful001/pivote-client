    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col font-sans">
      ...
      <header className="border-b border-slate-200/60 bg-white/80 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <Link
              to="/login"
              className="text-[#0d1e43] hover:text-[#10b981] font-semibold text-sm transition"
            >
              Voter Login
            </Link>
            <Link
              to="/admin/login"
              className="bg-[#0d1e43] hover:bg-slate-900 text-white font-semibold px-4 py-2 rounded-xl text-sm transition shadow-sm"
            >
              Admin Portal
            </Link>
          </div>
      ...
      <main className="flex-1 flex flex-col justify-center py-20 px-6 max-w-5xl mx-auto text-center space-y-12">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          ...
          <Link
            to="/login"
            className="w-full sm:w-auto border border-slate-200 bg-white hover:bg-slate-50 text-[#0d1e43] font-semibold px-8 py-4 rounded-xl transition text-base text-center"
          >
            Vote in a Program
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-16">
          <div className="bg-white p-8 rounded-2xl border border-slate-200/50 shadow-sm text-left space-y-4">
            ...
            <p className="text-slate-500 text-sm leading-relaxed">
              Every participant goes through verification, ensuring one secure vote per person with
              no double voting.
            </p>
          </div>
