...
import { useCreateProgram, type Program } from "../../../../api/program";
import {
...
export default function AdminCreateProgram() {
  ...
  // Created program id — unlocks candidates section after save
  ...
  const { data: existingCandidates, isLoading: loadingCandidates } = useCandidatesByProgram(
    createdProgramId ?? ""
  );
  ...
  const programSaved = !!createdProgramId;
  ...
  const handleSaveProgram = (e: React.FormEvent) => {
    ...
    // prevent re-creating if already saved
    if (programSaved) return;
    ...
    createProgramMutation.mutate(
      ...
      },
      {
        onSuccess: (data: Program) => {
          toast.success("Program created! Now add candidates below.");
  ...
  const handleAddCandidate = () => {
    ...
    createCandidateMutation.mutate(
      { name: candidateName.trim(), program_id: createdProgramId! },
      {
        onSuccess: () => {
  ...
  const handleUpdateCandidate = (id: string) => {
    ...
    updateCandidateMutation.mutate(
      { id, name: editingCandidateName.trim(), program_id: createdProgramId! },
      {
        onSuccess: () => {
  ...
  return (
    <div className="space-y-8 max-w-2xl mx-auto">
        ...
        <form onSubmit={handleSaveProgram} className="space-y-5">
          <div>
            ...
            <input
              ...
              disabled={programSaved}
              placeholder="e.g. Presidential Election"
              className="w-full bg-white border border-slate-200 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 rounded-xl py-3 px-4 text-[#0d1e43] placeholder-slate-400 transition outline-none disabled:opacity-60 disabled:cursor-not-allowed"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full bg-white border border-slate-200 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 rounded-xl py-3 px-4 text-[#0d1e43] placeholder-slate-400 transition outline-none resize-none disabled:opacity-60 disabled:cursor-not-allowed"
            />
          </div>
              ...
              {[
                ...
                <div key={label}>
                  ...
                  <input
                    ...
                    className="w-full bg-white border border-slate-200 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 rounded-xl py-3 px-4 text-[#0d1e43] transition outline-none disabled:opacity-60 disabled:cursor-not-allowed"
                  />
                </div>
              ))}
            </div>
          ...
          {programSaved && (
            <div className="flex items-center gap-2 text-emerald-600 text-sm font-semibold pt-2">
              ...
              Program saved — add candidates below
            </div>
          )}
        </form>
      </div>
      ...
      <div
        className={`bg-white border rounded-2xl p-6 shadow-sm transition-opacity ${
          programSaved
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              {programSaved
                ? "Add candidates to this program."
                : "Save the program above to unlock this section."}
            </p>
          </div>
        ...
        {programSaved && existingCandidates && existingCandidates.length > 0 && (
